...
const loadQuestions = async () => {
    const files = ['test2.json', 'test3.json', 'test4.json', 'test5.json'];
    try {
        ...
        allQuestions.value.test2 = results[0]; // Questions from test2.json
        allQuestions.value.others = results.slice(1).flat(); // Questions from other files
...
const selectQuestions = () => {
    // Select 15 random questions from test2.json
    const test2Questions = [...allQuestions.value.test2].sort(() => 0.5 - Math.random()).slice(0, 18);

    // Select 35 random questions from the other files
    const otherQuestions = [...allQuestions.value.others].sort(() => 0.5 - Math.random()).slice(0, 32);

    // Combine and shuffle all selected questions
    displayedQuestions.value = [...test2Questions, ...otherQuestions].sort(() => 0.5 - Math.random());
};
...
const score = computed(() => {
    return userAnswers.value.filter((answer, index) => answer === displayedQuestions.value[index].correctAnswer).length;
});
...
const progressPercentage = computed(() => {
    return ((currentQuestionIndex.value + 1) / displayedQuestions.value.length) * 100;
});

const isAnswerCorrect = (index) => {
    return userAnswers.value[index] === displayedQuestions.value[index].correctAnswer;
};
</script>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
              ...
              <p class="text-gray-600 dark:text-gray-400">Loading questions...</p>
                        ...
                        <div v-else-if="!testStarted" class="text-left">
                            <h1 class="text-3xl font-bold mb-6 text-gray-900 dark:text-gray-100 text-center">Full
                                Practice Test:
                                50 Questions</h1>
                            <div class="space-y-4 text-gray-600 dark:text-gray-400">
                                <p class="mb-4">You will have <span
                                        class="font-semibold text-gray-800 dark:text-gray-200">15
                                        minutes</span> to complete the test. The assessment measures your aptitude in
                                    areas such
                                    as critical thinking and problem-solving. This helps you demonstrate your potential
                                    to
                                    employers, beyond what appears in your resume.</p>
                                ...
                                <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg">
                                    ...
                                    <ul class="list-disc list-inside space-y-2">
                                        ...
                                        <li>It is unlikely that you will finish all 50 questions before time expires,
                                            but do
                                            your best to answer as many as you can.</li>
                                    </ul>
                                </div>
                                ...
                                <div class="bg-yellow-50 dark:bg-yellow-900 p-4 rounded-lg">
                                    ...
                                    <ul class="list-disc list-inside space-y-2 text-yellow-700 dark:text-yellow-300">
                                        ...
                                        <li>Leaving the assessment window or switching your browser tab will cause you
                                            to be
                                            locked out.</li>
                        ...
                        <div v-else-if="testStarted && !testCompleted" class="text-center">
                            ...
                            <Question :question="displayedQuestions[currentQuestionIndex]"
                                :question-number="currentQuestionIndex + 1" :total-questions="displayedQuestions.length"
                        ...
                        <div v-else class="text-center">
                            ...
                            <div v-for="(question, index) in displayedQuestions" :key="question.id" class="mb-8 text-left">
