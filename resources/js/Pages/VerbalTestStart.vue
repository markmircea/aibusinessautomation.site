...
const fetchNextQuestion = async () => {
  ...
  try {
    const response = await axios.post('/api/generate-question', {
      prompt: `Generate a verbal reasoning question with multiple choice answers. Follow these steps:

1. First, create a question from ONE of these categories (randomly selected):
   - Synonyms and Antonyms
   - Analogies
   - Sentence Completion
   - Reading Comprehension (without external passages)
...
3. Create an explanation object that MUST follow this EXACT structure:
   {
     "correctAnswerExplanation": "A detailed explanation of why the answer is correct, including relevant definitions, word relationships, or contextual clues",
     "commonMisconceptions": [
       "First common error in understanding or interpretation that might lead to a wrong answer",
       "Second common verbal reasoning mistake or misunderstanding"
     ],
     "keyConcepts": [
       "First key verbal reasoning skill or concept tested in this problem",
       "Second key language principle or technique needed"
     ]
   }
